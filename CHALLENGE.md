...
- **差別化:** SVG内部を書き換えず、利用目的から汎用モーションを絞り込み、実際の発火条件に近いプレビューとHTML・CSSを同時に生成します。危険または不正なSVGは自動修正せず拒否します。
- **主な機能:** 内蔵SVGアイコン18種類、任意SVG入力と安全検証、5用途、汎用CSSモーション12種類、5発火条件、モーション設定、sandbox iframeプレビュー、HTML・CSS個別コピー、アクセシビリティ案内
