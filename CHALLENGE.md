...
- **差別化:** 1秒ずつの単純減算ではなく、終了予定時刻と現在時刻の差分から残り時間を算出し、更新が遅れた後も実際の経過時間を反映します。
- **主な機能:** 作業・休憩時間の設定と検証、開始、一時停止、再開、リセット、手動・自動フェーズ切り替え、進捗表示、重要な状態変化のスクリーンリーダー通知、実行中の`document.title`更新
- **GitHub上のパス:** `app/works/04-pomodoro-mini/`
- **想定公開URL:** https://ai-build-challenge.vercel.app/works/04-pomodoro-mini
- **検証結果:** `npm run lint`、`npm run typecheck`、`npm run test`（5ファイル・135件）、`npm run build`、`git diff --check`に成功しました。
- **ブラウザ確認:** ローカル本番サーバーをChromeで開き、幅390pxと1440pxで初期表示、1〜120分の表示、設定変更、空欄・0・負数・小数・上限超過・非数値、エラー解消、最初の無効欄へのフォーカス移動、開始・一時停止・再開・リセット、実行中のフェーズ切り替え、1分経過後の自動切り替えと停止、`document.title`の更新と復元、Tab・Shift+Tab・Enter・Space、フォーカス表示、進捗のARIA属性、横スクロール、秒更新前後の主要領域の寸法、コンソールを確認しました。自動操作環境では別タブを開いても元ページが非表示状態にならず、`visibilitychange`による復帰補正は未確認です。時刻差分による大きな遅延の補正は単体テストで確認しました。
- **既知の制約:** 音声通知やOS通知は行いません。ブラウザを閉じた状態では動作しません。端末スリープ中は画面を更新しませんが、ページが残っていれば復帰後に実時間との差分から補正します。入力内容やタイマー状態、履歴は保存しません。
- **学んだこと:** 終了予定時刻を状態として保持し、残り時間の算出を純粋関数へ分離すると、更新間隔や背景タブの間引きに依存しない時間経過を境界値テストで保証できます。
- **次回への改善点:** 自動操作環境でもページの非表示化と復帰を確実に再現できるブラウザテスト手段を、実装前に確認します。
