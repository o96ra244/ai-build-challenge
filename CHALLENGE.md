...
- **ブラウザ確認:** Chromeのローカル本番ビルドを幅390pxと1440pxで確認しました。両幅で設定、プレビュー、結果が縦1カラムに並び、横スクロールはありません。全5数値入力でArrowUp／ArrowDownとネイティブのスピン矢印による1刻みの増減、上下限で値を超えないことを確認しました。全5入力の空欄・小数・負数・範囲外ではフォーカスを維持してエラーを表示し、直前の有効な結果とコードを維持してNaN・Infinityを表示しません。有効値へ戻すと、プレビュー、列数、カード幅、最大幅、切り替わり表、HTML、CSSへ追加操作なしで反映されます。`auto-fit`／`auto-fill`も数値エラー中を含めて即時反映され、前回修正した到達可能列数の表と入力ヒント・エラーの`aria-describedby`関連付けも維持しています。送信ボタンがないこと、ラジオのSpace、スライダーのEnd、HTML・CSSコピー、初期値へのリセット、focus-visible、コンソールのerror・warningなしも確認しました。
