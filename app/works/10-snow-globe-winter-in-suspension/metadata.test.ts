...
const workDirectory = path.join(process.cwd(), "app/works/10-snow-globe-winter-in-suspension");
const pageSource = fs.readFileSync(path.join(workDirectory, "page.tsx"), "utf8");
const ogImage = fs.readFileSync(path.join(process.cwd(), "public/og/10-snow-globe-winter-in-suspension.png"));

describe("Work 10 SEO and social metadata", () => {
  it("keeps the production canonical and social card settings", () => {
    expect(pageSource).toContain("https://ai-build-challenge.vercel.app/works/10-snow-globe-winter-in-suspension");
    expect(pageSource).toContain("/og/10-snow-globe-winter-in-suspension.png");
  ...
  it("keeps the final production frame as a 1200x630 PNG", () => {
